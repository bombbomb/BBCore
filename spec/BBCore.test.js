...
describe("BBCore", function () {
  ...
  it("__mergeProperties against Class", function () {
    ...
    expect(
      JSON.stringify(new BaseClass({ third: { two: "b" }, second: "fine" }))
...
describe("BBCore.api", function () {
      ...
      info: [
        {
          ...
          height: "480",
          width: "640",
          created: "2/06/15 10:33:05 am",
          vidUrl: "videoDeliveryUrl",
        },
      ],
    },
    responseFailure: {
  ...
  xit("sendRequest - with ajax request error", function () {
    // Remove jquery dependency from bbcore.api onerror before fixing this test
    const errorParamCallbackSpy = jest.fn();
    bbCore.sendRequest(
      "GetVideos",
      { video_id: testGuid },
      successCallbackSpy,
      errorParamCallbackSpy
    );

    onerror(/* Call with fake xhr error response? */);

    expect(errorParamCallbackSpy).toHaveBeenCalled();
    expect(successCallbackSpy).not.toHaveBeenCalled();
  });
  ...
  xit("sendRequest - with ajax request error when last response was success", function () {
    ...
    // This test is broken until jquery specific xhr response is removed from onerror
    ...
    expect(errorParamCallbackSpy).toHaveBeenCalledWith(
      bbCore,
      xhrResult.responseJSON
    );
...
describe("BBCore.auth", function () {
  ...
  it("login", function () {
    ...
    callFakeOnloadWithApiResponse(JSON.stringify(result.authenticationSuccess));
  ...
  it("validateSession - fails", function () {
    ...
    callFakeOnloadWithApiResponse(
      JSON.stringify({ status: "success", info: {} })
    );
  ...
  it("clearKey", function () {
    ...
    expect(bbCore.getKey()).toBeUndefined();
  });

  it("getOAuthTokenForRequest", function () {
    var fakeTokenPayload = { token_type: "", access_token: "" };

    spyOn(bbCore, "getOAuthPayload").and.returnValue(
      JSON.stringify(fakeTokenPayload)
    );
    ...
    expect(tokenForRequest).toBe(
      fakeTokenPayload.token_type + " " + fakeTokenPayload.access_token
    );
  });
});
...
describe("BBCore.contacts", function () {
  ...
  it("getLists", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.any(Object),
      successCallbackSpy
    );
  });
  ...
  it("createList", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ name: listName }),
      successCallbackSpy
    );
  });
  ...
  it("getContact", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ contact_id: contactId }),
      successCallbackSpy
    );
  });
  ...
  it("getListContacts", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ list_id: listId }),
      successCallbackSpy
    );
  });
  ...
  it("addContact", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ eml: "test@test.com" }),
      successCallbackSpy
    );
  });
  ...
  it("bulkAddContacts: with contacts", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ contacts: '[{"eml":"test@test.com"}]' }),
      successCallbackSpy
    );
  });
  ...
  it("updateContact", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ firstname: "Timmy" }),
      successCallbackSpy
    );
  });
  ...
  it("addContactImportAddress", function () {
    bbCore.addContactImportAddress(
      { importAddrCode: 1, importAddrName: "name" },
      successCallbackSpy
    );

    expect(bbCore.onError).not.toHaveBeenCalled();
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ importAddrCode: 1, importAddrName: "name" }),
      successCallbackSpy
    );
  });
  ...
  it("addContactImportAddress: with an undefined importAddrCode calls BBCore.onError", function () {
    ...
    expect(bbCore.onError).toHaveBeenCalled();
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ importAddrName: "name" }),
      successCallbackSpy
    );
  });
  ...
  it("addContactImportAddress: with an undefined importAddrName calls BBCore.onError", function () {
    ...
    expect(bbCore.onError).toHaveBeenCalled();
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ importAddrCode: 1 }),
      successCallbackSpy
    );
  });
...
describe("BBCore.extras", function () {
  ...
  it("getClientIntegrations with Options", function () {
    bbCore.getClientIntegrations(
      { integrationCode: "GAPPS" },
      successCallbackSpy
    );
...
describe("BBCore.video", function () {
  ...
  it("getVideoDeliveryUrl", function () {
    var expected =
      "http://dev.bbemaildelivery.com/bbext/?p=video_land&id=" +
      testGuid +
      "&autoplay=0";

    var actual = bbCore.getVideoDeliveryUrl({
      video_id: testGuid,
      autoplay: 0,
    });
  ...
  it("getVideo", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ video_id: validVideoId }),
      successCallbackSpy
    );
  });
  ...
  it("getVideos", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ method: "GetVideos" }),
      successCallbackSpy
    );
  });
  ...
  it("getVideoStatus", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ id: validVideoId }),
      successCallbackSpy
    );
  });
  ...
  it("deleteVideo", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ videoId: validVideoId }),
      successCallbackSpy
    );
  });
  ...
  it("videoQuickSend: using current video id", function () {
    ...
    bbCore.setVideoId(validVideoId);
    bbCore.videoQuickSend(
      { subject: "quick send subject", email_address: "test@test.com" },
      successCallbackSpy
    );
  ...
  it("videoQuickSend: with undefined video_id and no current video id generates a new video id", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ video_id: testGuid }),
      successCallbackSpy
    );
  });
...
describe("BBCore.videoRecorder", function () {
  var result = {
    withOptionsSuccess: {
      ...
      info: {
        ...
        client_id: "<Guid>",
        vid_id: "<Guid>",
        content: "<Video Recorder Html>",
        width: 640,
        height: 480,
        https: true,
      },
    },
    withDefaultOptionsSuccess: {
      status: "success",
      info: {
        user_id: "<Guid>",
        email: "test@test.com",
  ...
  xit("getEmbeddedRecorderUrl with Legacy Access Token", function () {
    ...
    var expectedUrl =
      apiServerUrl +
      "/app/?module=login&actn=login&api_key=" +
      apiKey +
      "&redir=" +
      btoa(apiServerUrl + "/app/?" + requestParams + "&vguid=" + testGuid);
  ...
  xit("getEmbeddedRecorderUrl using OAuth", function () {
    ...
    var expectedUrl =
      apiServerUrl + "/app/?" + requestParams + "&vguid=" + testGuid;
    ...
    spyOn(bbCore, "getVideoRecorder").and.callFake(function (opts, onComplete) {
      onComplete({
        info: {
          content: '<div><iframe src="' + expectedUrl + '"></iframe></div>',
        },
      });
    });
  ...
  xit("getVideoRecorder: without options", function () {
    ...
    expect(successCallbackSpy).toHaveBeenCalledWith(
      result.withDefaultOptionsSuccess
    );
  });
  ...
  xit("startVideoRecorder", function () {
    var options = {
      width: 640,
      height: 480,
      recorderLoaded: jasmine.createSpy(),
    };
  ...
  xit("startVideoRecorder: with default options", function () {
    ...
    expect(bbCore.getVideoRecorder).toHaveBeenCalledWith(
      jasmine.any(Object),
  ...
  xit("startVideoRecorder: specify recordComplete callback in options", function () {
    ...
    expect(bbCore.liveStreamStartRecord).toHaveBeenCalledWith(
      streamName,
      videoFilename
    );
  ...
  it("liveStreamStartRecord", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ streamname: streamName, filename: filename })
    );
  });
  ...
  it("liveStreamStopRecord", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.objectContaining({ streamname: streamName })
    );
  });
  ...
  it("saveRecording", function () {
    ...
    expect(bbCore.sendRequest).toHaveBeenCalledWith(
      jasmine.any(Object),
      jasmine.objectContaining({ vid_id: testGuid }),
      jasmine.any(Function)
    );
  });
